...
export const ActivityCarousel: React.FC<ActivityCarouselProps> = ({
  ...
  // Continuous auto-slide every 2 seconds without pause
  useEffect(() => {
  ...
  const displaySubtitle = language === 'ja'
    ? (currentItem.subtitleJapanese || '学内活動・文化発信')
    : (currentItem.subtitle || 'CAMPUS ACTIVITY · INDONESIA');
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-xs font-serif tracking-[0.15em] sm:tracking-[0.2em] text-[#66645F] uppercase font-semibold">
          ...
          <span>
            {language === 'ja'
              ? '学内活動・地域貢献・文化発信'
              : 'ACTIVITIES & CAMPUS LIFE · INDONESIA'}
          </span>
        </div>
      ...
      {/* ═══ VIEW MODE: CAROUSEL ═══ */}
      {viewMode === 'carousel' ? (
        <div className="relative bg-[#FFFFFF] border border-[#D9D5CC] rounded-[3px] p-6 sm:p-8 shadow-[0_4px_20px_-8px_rgba(38,59,80,0.06)] hover:shadow-[0_12px_32px_-10px_rgba(38,59,80,0.12)] transition-shadow duration-300">
          {/* Main Stage Grid: Photo on Left, Synchronized Narrative on Right */}
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
            {/* LEFT: Photo Showcase Stage */}
            <div className="md:col-span-5 relative flex justify-center">
              <div className="relative w-full max-w-md aspect-[4/3] overflow-hidden rounded-[2px] bg-[#EFEBE3] border border-[#D9D5CC] group/photo">
                ...
                <div className="absolute top-3 left-3 right-3 flex items-center justify-between z-10">
                  <span className="px-2.5 py-1 bg-[#66645F]/90 backdrop-blur-sm text-[#F7F5F0] text-[10px] font-mono tracking-widest uppercase rounded-[2px]">
                    {language === 'ja' ? '学内活動' : 'ACTIVITY'}
                  </span>
          ...
          <div className="mt-8 pt-6 border-t border-[#D9D5CC]">
            <div className="text-[11px] font-serif uppercase tracking-[0.2em] text-[#66645F] mb-3">
              {language === 'ja' ? '写真一覧（クリックで選択）' : 'All Activities · Click to View'}
            </div>
              ...
              {items.map((item, idx) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => go(idx)}
                    className={`relative aspect-[4/3] rounded-[2px] overflow-hidden border transition-all duration-200 cursor-pointer ${
                      isActive
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item, idx) => {
            ...
            const itemSubtitle = language === 'ja'
              ? (item.subtitleJapanese || '学内活動・文化発信')
              : (item.subtitle || 'CAMPUS ACTIVITY · INDONESIA');
            ...
            return (
              ...
              >
                ...
                <div className="p-5 space-y-2 flex flex-col justify-between flex-1">
                  <div>
                    {itemSubtitle && (
                    ...
                    <h4 className="font-serif text-base sm:text-lg text-[#1C1C1C] group-hover:text-[#263B50] transition-colors leading-snug">
                      {itemTitle}
                    </h4>
                    <p className="font-sans text-xs text-[#66645F] leading-relaxed mt-1">
                      {itemCaption}
                    </p>
                  </div>

                  <div className="pt-3 flex items-center justify-between text-xs font-sans text-[#66645F] border-t border-[#D9D5CC]/60 mt-2">
                    <span className="flex items-center gap-1 truncate max-w-[150px]">
                      <MapPin className="w-3.5 h-3.5 text-[#263B50] shrink-0" />
